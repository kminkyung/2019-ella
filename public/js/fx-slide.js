...
var FxSlide = (function(){
	function FxSlide(obj) {
		...
		this.interval = null;
		this.arr = [];
		this.startInit(this); // 객체생성시 한번만 실행(이벤트)
	...
	FxSlide.prototype.startInit = function(obj) {
        ...
        // 기존에 세팅되어있는 그림을 바꾸는 곳
				if(obj.dir == 0) {
					obj.slides.children().eq(0).html($(obj.slide[obj.now]).html());
				}
				else {
          obj.slides.children().eq(2).html($(obj.slide[obj.now]).html());
				}
				obj.ani(obj, true);
			});
		}
